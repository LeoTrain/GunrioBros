...
public class Enemy : MonoBehaviour
{
    ...
    public float playerFindRadius = 3f;
    ...
    void Update()
    {
        if (this.IsPlayerinRadius() || this.isPlayerFound)
        {
            this.CheckToSwitchPlayer();
            this.MoveToPlayer();
        }
        else
        {
    ...
    {
        if (Vector2.Distance(this.transform.position, GameObject.Find("Player").transform.position) < this.playerFindRadius)
        {
    ...
    void CheckToSwitch()
    {
        if (this.IsLimitRight() && this.current_direction != Vector2.left)
        {
            ...
            this.transform.position = new Vector2(this.transform.position.x + (this.current_direction.x * 0.02f), this.transform.position.y);
        }
